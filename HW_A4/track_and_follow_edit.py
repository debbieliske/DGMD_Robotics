...
This code example is based in part on the 'offboard_position_ned.py' and the 'telemetry_takeoff_and_land.py'
...
async def get_target_coordinates(vid_cam):
    ...
    #tt_, contours, _ = cv2.findContours(mask,  cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    #ttcontours, hierarchy= cv2.findContours(mask,  cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
